...
check_min_version() {
    ...
    if [ "$id" = "debian" ]; then
        if [ -z "${debian_supported[$codename]}" ]; then
            echo "ERROR: Debian release '$codename' is not supported. Minimum is bookworm (12)." >&2
            exit 1
        fi
    elif [ "$id" = "ubuntu" ]; then
        if [ -z "${ubuntu_supported[$codename]}" ]; then
            echo "ERROR: Ubuntu release '$codename' is not supported. Use jammy, noble, or resolute." >&2
            exit 1
        fi
    else
