...
- **Fixed empty `.deb` packages produced after successful compilation.** The root cause was `buildRepo()` invoking `just build` instead of `just build-release --frozen`. All COSMIC `justfile` install targets reference binaries under `target/release/`; invoking `just build` places binaries in `target/debug/` and causes the subsequent `just rootdir=<dest> install` to silently install nothing, producing an empty staging directory and therefore an empty `.deb`. The fix aligns with the invocation used by all upstream Arch Linux PKGBUILDs: `just build-release --frozen`.
- **Fixed empty `.deb` packages for Makefile-based components** (`cosmic-comp`, `cosmic-settings-daemon`, `cosmic-workspaces-epoch`, `xdg-desktop-portal-cosmic`). The `installToStage()` function for these components was calling `make DESTDIR=<dest> install` without `prefix=/usr` or `libexecdir=/usr/lib`. Without an explicit prefix, these Makefiles install to `/usr/local` inside the staging directory, which places files outside the paths that `dpkg-deb` packages as intended content. The fix passes `prefix=/usr libexecdir=/usr/lib DESTDIR=<dest>` consistently with upstream packaging references.
- **Fixed Makefile build invocation for Makefile-based components.** `buildRepo()` now passes `ARGS=--frozen --release` as a Make variable override when no `justfile` is present, causing the Cargo invocation embedded in these Makefiles to build in release mode with the locked dependency set.
- **Added `RUSTFLAGS` propagation** via a new `runEnv()` helper and `buildEnv()` function. All build invocations now set `RUSTFLAGS=-C link-arg=-fuse-ld=lld`, matching upstream PKGBUILD practice. This ensures the `lld` linker (already a declared build dependency) is used and avoids link failures on systems where the default linker cannot resolve all Wayland/dbus symbol references at link time.
- **Added `runEnv()` helper** to `main.go` for executing subprocesses with additional environment variables appended on top of the inherited environment, without modifying the parent process environment.
- Fixed `scripts/uninstall.sh` package list: removed the non-existent `cosmic-theme-extra` entry (the upstream repository `pop-os/cosmic-theme-extra` does not exist on GitHub), and added the previously missing `cosmic-initial-setup`, `pop-launcher`, and `cosmic-desktop` (meta package) entries.
